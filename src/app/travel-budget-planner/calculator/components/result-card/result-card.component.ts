  ...
  <div class="calc-result-card">
    <div class="country-info-box">
      <div class="country-info-photo" [class.photo-loading]="!countryPhotoUrl">
        @if (countryPhotoUrl) {
          <img [src]="countryPhotoUrl" [alt]="getCountryLabel(selectedCountry)" loading="eager" decoding="async">
        }
      </div>
      <div class="country-info-header">
        <span class="country-info-flag" aria-hidden="true">
          @if (!isFlagBroken(selectedCountry.code)) {
            <img [src]="getFlagApiUrl(selectedCountry.code)" [alt]="''"
              loading="lazy" decoding="async" (error)="markFlagBroken(selectedCountry.code)">
          }
          @if (isFlagBroken(selectedCountry.code)) {
            <span>{{ getFallbackFlagEmoji(selectedCountry.code) }}</span>
          }
        </span>
    ...
    @if (selectedCountryComparison) {
      <section class="country-compare-box">
        ...
        <div class="compare-total-card" [class.expanded]="compareExpanded">
          <div class="compare-total-header">
            ...
            @if (result) {
              <button class="compare-toggle-btn" (click)="compareExpanded = !compareExpanded"
